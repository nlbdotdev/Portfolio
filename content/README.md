...
Timeline ordering uses the start/release value, then a stable `order` tie-breaker. Unknown dates sort last. Entries before 2020, explicitly archived entries, and undated entries are folded initially. Track buttons filter the shared timeline with a fade-and-collapse transition; searching includes the folded archive.
